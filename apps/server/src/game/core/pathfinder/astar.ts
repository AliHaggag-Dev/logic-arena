...
export class AStarProtocol {
  ...
  performAStar(
    ...
    const sC = Math.min(PATH_CONFIG.COLS - 1, Math.max(0, Math.floor(startX  / PATH_CONFIG.CELL)));
    const sR = Math.min(PATH_CONFIG.ROWS - 1, Math.max(0, Math.floor(startY  / PATH_CONFIG.CELL)));
    let   tC = Math.min(PATH_CONFIG.COLS - 1, Math.max(0, Math.floor(targetX / PATH_CONFIG.CELL)));
    let   tR = Math.min(PATH_CONFIG.ROWS - 1, Math.max(0, Math.floor(targetY / PATH_CONFIG.CELL)));
    ...
    while (heap.size > 0 && iters < PATH_CONFIG.MAX_ITER) {
      ...
      for (const [dr, dc] of DIRS) {
        ...
        if (nr < 0 || nr >= PATH_CONFIG.ROWS || nc < 0 || nc >= PATH_CONFIG.COLS) continue;
        if (this.gridBuilder.impassable[nr][nc]) continue;

        // Diagonal corner-cut prevention to stop robots clipping geometry apexes
        if (dr !== 0 && dc !== 0) {
          if (this.gridBuilder.impassable[cur.r + dr]?.[cur.c] || this.gridBuilder.impassable[cur.r]?.[cur.c + dc]) {
            continue;
          }
        }
  ...
  private reconstruct(
    ...
    while (k !== undefined && k !== -1) {
      raw.push({ r: Math.floor(k / PATH_CONFIG.COLS), c: k % PATH_CONFIG.COLS });
