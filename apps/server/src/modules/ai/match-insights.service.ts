...
export class MatchInsightsService {
  ...
  async generateForMatch(matchId: string, userId: string): Promise<number> {
    ...
    if (this.generationLocks.has(lockKey)) {
      this.logger.log(`Generation already in progress for match ${matchId} and user ${userId}`);
      return 0;
    }
    ...
    if (existingInsightsCount > 0) {
      this.logger.log(`Insights already generated for match ${matchId} and user ${userId}`);
      return 0;
    }
    ...
    if (match.replayData && typeof match.replayData === 'object' && !Array.isArray(match.replayData)) {
    ...
    const aiInsights = await this.aiService.generateMatchInsights(scriptCode, {
      isWinner,
    ...
    if (!aiInsights || aiInsights.length === 0) {
      this.logger.warn(`Failed to generate dynamic AI insights for match ${matchId}. Using fallback.`);
      // Minimal fallback just in case AI fails
      aiInsights.push({
        title: 'Match Analysis',
        content: isWinner ? 'Great job! Your script performed well.' : 'Keep trying! Adjust your logic and try again.',
    ...
    for (const insight of aiInsights) {
      ...
      this.logger.log(`Generated ${aiInsights.length} insights for match ${matchId}`);
