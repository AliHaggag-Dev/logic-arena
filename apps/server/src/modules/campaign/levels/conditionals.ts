...
export const CONDITIONALS_LEVELS: CampaignLevel[] = [
  {
    ...
    hints: [
      'It only fires when VISIBLE_ENEMY_COUNT > 0. Approach from outside its FOV cone, then strike while it patrols rightward.',
...
END`,
  ...
  },
  {
    ...
    difficulty: 'HARD',
    pointsReward: D.HARD,
    conceptTaught: 'Damage-triggered polarity toggle',
    description:
    ...
    hints: [
      'Polarity 1 makes it strafe predictably. Hit it once to flip polarity, then exploit the strafe pattern before the next flip.',
      'In polarity 1 it strafes RIGHT continuously. Position yourself to the left — it strafes into your fire. Hit it a second time to flip back to polarity 0 (patrol mode), then finish it quickly.',
      'Track damage hits with a counter: SET hits = hits + 1 before firing. On even hits (0,2,4) it will be in polarity 1 (strafe right). Time your burst attacks for these windows.',
    ],
    enemyScript: `IF NOT init THEN
  SET pol = 0
  SET prev_hp = 100
