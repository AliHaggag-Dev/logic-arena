...
const RegisterPage = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    setStatus({ message: "INITIALIZING UPLINK...", type: null });

    try {
      await apiClient.post("/auth/register", { email, username, password });
      setStatus({ message: "[SYS] REGISTRATION SUCCESSFUL. REROUTING...", type: "success" });
      ...
      setStatus({
        message: `[ERR] REGISTRATION FAILED: ${error.response?.data?.message || error.message}`,
        type: "error"
      });
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-950 font-mono selection:bg-cyan-500/30 relative overflow-hidden">
      ...
      <div className="w-full max-w-md p-8 bg-black/60 backdrop-blur-xl border border-cyan-900/60 rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] relative z-20">
        ...
        <div className="absolute top-2 right-3 text-[9px] text-cyan-500/50 tracking-widest pointer-events-none">SECURE_NODE_V1</div>
        ...
        <div className="mb-8 text-center">
          ...
          <h2 className="text-purple-400/80 text-xs tracking-widest uppercase">
        ...
        <form onSubmit={handleRegister} className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-[10px] text-cyan-600 uppercase tracking-[0.2em] font-bold ml-1" htmlFor="email">
              Comms Link (Email)
            </label>
            <input
              type="email"
          ...
          {status.message && (
            <div className={`p-3 rounded border text-xs break-words ${status.type === 'error' ? 'bg-red-950/40 border-red-900/50 text-red-400' :
              status.type === 'success' ? 'bg-green-950/40 border-green-900/50 text-green-400' :
          ...
          <div className="mt-2 flex flex-col gap-4">
            <button
              type="submit"
              disabled={isLoading}
              className="w-full py-3 bg-purple-600/20 border border-purple-500/50 text-purple-300 font-bold text-xs hover:bg-purple-600/40 hover:text-white transition-all rounded-lg uppercase tracking-[0.15em] shadow-[0_0_15px_rgba(168,85,247,0.25)] disabled:opacity-50 disabled:cursor-not-allowed"
            >
