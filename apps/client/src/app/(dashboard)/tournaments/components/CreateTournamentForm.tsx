  ...
  return (
    <div className={`mb-8 ${isMobile ? "p-5" : "p-6"} rounded-xl bg-card/20 border border-accent/20 backdrop-blur-xl animate-[fadeIn_0.25s_ease] flex ${isMobile ? "flex-col" : "gap-4"} items-stretch relative overflow-hidden group`}>
      ...
      <div className="flex-1 flex flex-col gap-2 relative z-10">
        <label className="text-[10px] font-black tracking-[0.3em] text-accent/40 uppercase pl-1 flex justify-between items-center">
          <span>// TOURNAMENT_ID</span>
          {isMobile && (
            <button
                onClick={onClose}
        ...
        <div className="relative">
            <input
                ...
                placeholder="SPECIFY_DESIGNATION..."
                className={`w-full ${isMobile ? "py-4 text-[13px]" : "py-3 text-[12px]"} px-4 bg-black/40 border border-accent/20 rounded-lg text-accent font-mono tracking-[0.1em] outline-none focus:border-accent/60 focus:bg-black/60 transition-all placeholder:opacity-20`}
            />
      ...
      <div className={`flex ${isMobile ? "mt-4 gap-2" : "items-end"} relative z-10`}>
        <button
            ...
            className={`flex-1 ${isMobile ? "py-4" : "py-3 px-8"} rounded-lg text-[10px] font-black tracking-[0.3em] font-mono transition-all duration-200 uppercase relative overflow-hidden ${
            creating ? 'cursor-wait opacity-50' : 'cursor-pointer'
            } ${!name.trim() ? "opacity-30 grayscale" : ""} ${
            hoveredBtn === "deploy"
        ...
        >
            {creating ? "DEPLOYING..." : "▶ DEPLOY_CIRCUIT"}
        </button>
