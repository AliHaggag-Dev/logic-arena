        ...
        <div className={`${isMobile ? "px-4" : "px-5"} pb-5 border-t border-amber-500/10 animate-in fade-in slide-in-from-top-1 duration-200`}>
          ...
          <div className="relative">
            ...
            <pre
              ...
              style={{
                color: '#fcd34d', // lighter amber for code
              }}
            >
              {example.code}
            </pre>
          ...
          <button
            ...
            className="mt-4 w-full py-3 text-[10px] font-black tracking-[0.3em] uppercase transition-all border rounded-lg shadow-[0_2px_10px_rgba(245,158,11,0.1)] active:scale-[0.98] text-amber-500 hover:bg-amber-500/10 border-amber-500/40 bg-amber-500/5"
          >
            ▶ LOAD_TO_CORE
          </button>
        </div>
          ...
          <div className="mt-4 rounded-xl border border-amber-500/20 bg-card/40 overflow-hidden">
            <div className="grid grid-cols-1 divide-y divide-amber-500/10">
              {ROTATION_SYSTEM_GUIDE.conflictRules.map((rule, idx) => (
                <div key={idx} className={`${isMobile ? "flex flex-col p-3 gap-2" : "grid grid-cols-[250px_1fr] p-3 gap-6 items-center"} hover:bg-amber-500/[0.02]`}>
