...
import { AlertTriangle, ArrowLeft, CheckCircle, Info, Lightbulb, Loader2, Send } from "lucide-react";
...
export default function FeatureRequestsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      await apiClient.post("/feedback/feature-requests", {
        title,
        description,
      ...
      <div className="relative z-10 max-w-2xl mx-auto px-4 sm:px-6 pt-16 sm:pt-24 pb-12 sm:pb-20">
        ...
        <header className="mb-10 relative rounded-2xl overflow-hidden" style={{ background: "rgba(var(--accent-rgb),0.04)", border: "1px solid rgba(var(--accent-rgb),0.18)", boxShadow: "inset 0 0 40px rgba(var(--accent-rgb),0.04)" }}>
          ...
          <div className="px-8 sm:px-10 py-8 relative z-10">
            <div className="flex items-center gap-2.5 mb-4">
              ...
              <span className="text-[9px] font-black tracking-[0.5em] uppercase" style={{ color: "rgba(var(--accent-rgb),0.5)", fontFamily: "var(--font-mono)" }}>Feedback</span>
            </div>
        ...
        <div className="rounded-2xl overflow-hidden bg-bg-secondary/50" style={{ border: "1px solid rgba(var(--accent-rgb),0.15)", backdropFilter: "blur(12px)" }}>
          <div className="px-6 sm:px-8 py-5 border-b flex items-center gap-3" style={{ borderColor: "rgba(var(--accent-rgb),0.1)" }}>
            ...
            <span className="text-[11px] font-black tracking-[0.3em] uppercase" style={{ color: "rgba(var(--accent-rgb),0.6)", fontFamily: "var(--font-mono)" }}>Request Details</span>
          </div>
          ...
          <div className="px-6 sm:px-8 py-7">
            {submitted ? (
              <div className="flex flex-col items-center gap-5 py-12 text-center">
                ...
                <div>
                  <h2 className="text-[15px] font-black tracking-[0.25em] uppercase mb-2" style={{ color: "var(--accent)", fontFamily: "var(--font-mono)" }}>Request Submitted</h2>
                  <p className="text-[13px] leading-[1.8]" style={{ color: "rgba(var(--accent-rgb),0.6)", fontFamily: "var(--font-mono)" }}>Your idea has been logged. We review all community submissions each sprint.</p>
                </div>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                <div>
                  <FieldLabel htmlFor="feature-title">// Feature Title</FieldLabel>
                  <StyledInput id="feature-title" type="text" placeholder="What should we build?" value={title} onChange={e => setTitle(e.target.value)} required disabled={loading} />
                </div>
                <div>
                  <FieldLabel htmlFor="feature-description">// Description</FieldLabel>
                  <StyledTextarea id="feature-description" placeholder="Describe the feature in detail. What problem does it solve?" value={description} onChange={e => setDescription(e.target.value)} required rows={4} disabled={loading} />
                </div>
                <div>
                  <FieldLabel htmlFor="feature-usecase">// Use Case</FieldLabel>
                  <StyledTextarea id="feature-usecase" placeholder="How would you use this? Walk us through a concrete scenario..." value={useCase} onChange={e => setUseCase(e.target.value)} required rows={3} disabled={loading} />
                </div>
                <div>
                  <FieldLabel htmlFor="feature-priority">// Priority</FieldLabel>
                  <CyberSelect id="feature-priority" value={priority} onChange={val => setPriority(val as Priority)} options={FEATURE_OPTIONS} placeholder="SELECT PRIORITY..." disabled={loading} />
                </div>
                {error && (
