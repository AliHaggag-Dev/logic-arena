...
function PaletteBlock({ definition, onAddBlock }: PaletteBlockProps): React.ReactElement {
  ...
  return (
    <button
      ...
      className="flex min-h-[58px] w-full flex-col items-start gap-0.5 rounded-xl px-3 py-2.5 text-left transition-[transform,background,border-color] duration-200 active:scale-[0.98]"
      style={{
...
export function BlockPalette({ onAddBlock, onClose }: BlockPaletteProps): React.ReactElement {
  ...
  return (
    <section
      ...
      className="flex min-h-0 flex-1 flex-col gap-2 overflow-hidden rounded-2xl p-2.5"
      style={{
        background: "color-mix(in srgb, var(--card) 12%, rgba(var(--arena-black-rgb),0.92))",
        border: "1px solid rgba(var(--arena-white-rgb),0.08)",
        boxShadow: "inset 0 1px 0 rgba(var(--arena-white-rgb),0.05), 0 24px 64px rgba(var(--arena-black-rgb),0.6)",
    ...
    >
      <div className="flex shrink-0 items-center justify-between gap-2 px-0.5">
        <div>
          <h2 className="text-sm font-bold" style={{ color: "var(--arena-white)" }}>Command Library</h2>
          <p className="text-[10px]" style={{ color: "rgba(var(--arena-white-rgb),0.5)" }}>
            {searchQuery.trim()
        ...
        {onClose && (
          <button
            ...
            style={{
              color: "rgba(var(--arena-white-rgb),0.62)",
      ...
      <div className="relative shrink-0">
        ...
        <input
          ...
          className="min-h-9 w-full rounded-xl py-1.5 pl-9 pr-3 text-xs outline-none transition-[border-color,box-shadow] duration-200"
          spellCheck={false}
          style={{
      ...
      {!searchQuery.trim() && (
        <div 
          className="flex shrink-0 gap-1.5 overflow-x-auto pb-1 scrollbar-hide overscroll-x-contain"
          ...
          {BLOCK_CATEGORIES.map((category) => {
            ...
            return (
              <button
                ...
                className="flex min-h-10 shrink-0 items-center justify-center rounded-xl px-3.5 text-[11px] font-semibold tracking-wide transition-all duration-200"
                style={{
      ...
      <div className="min-h-0 flex-1 overflow-hidden">
        <div ref={scrollRef} className="grid h-full w-full grid-cols-2 content-start gap-2 overflow-y-auto overscroll-contain px-0.5 pt-1">
